    ...
    <div className="space-y-5">
      {paragraphs.map((p, i) => {
        ...
        if (/^#{2,3}\s/.test(trimmed)) {
          ...
          if (level === 2) {
            return <h2 key={i} className="text-headline-md font-semibold text-on-surface mt-8 mb-3">{text}</h2>;
          }
        ...
        if (trimmed.startsWith('> ')) {
          ...
          return (
            <blockquote key={i} className="border-l-4 border-primary pl-6 italic text-on-surface-variant text-body-md">
              {renderInline(text)}
            </blockquote>
          );
        }
